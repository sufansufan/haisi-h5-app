<template>
  <div class="show-box" :class="{disabled: disabled, active: isShowList}" :style="style_Container">
    ...
    <span
      ...
      $click="onClear"
    >
      <span class="iconfont iconshanchu1 clear" />
    </span>
    ...
    >
      <span class="popper__arrow" /> <!-- 列表框左上角的空心小三角 -->
      <scroll-view
        class="list"
      ...
      >
        ...
        <div v-show="innerList.length==0" class="data-state item">
          无数据
        </div>
        <!-- <slot></slot> -->
      </scroll-view>
...
export default {
  ...
  computed: {
    ...
    innerList() { // 转换列表的数据格式
      const arr = []; const orginArr = this.list
      orginArr.forEach((val, index) => {
        const value = typeof val === 'object' && 'value' in val ? val.value : val
        const isDisabled = typeof val === 'object' && val.disabled == true
        arr.push({
          isActive: false,
          value: value,
          disabled: isDisabled
  ...
  methods: {
    ...
    changeActiveIndex(value_index) { // 改变列表中的活动项
      ...
      let activeIndex = value_index; let value = value_index
